# F24 Arduino Example 07: DS3231 Real‑Time Clock
...
## File Structure
- `src/F24_07_ds3231_rtc.ino` – Arduino source code.
- `precompiled/F24_07_ds3231_rtc.bin` – Precompiled firmware binary for ESP32‑S3.

## Using the Precompiled Binary
Flash `precompiled/F24_07_ds3231_rtc.bin` to address `0x0` using ESP Flash Download Tool or `esptool.py`.
...
## Expected Behavior
1. After reset, the Serial Monitor (115200 baud) shows the current RTC time:
Current time is: 2025-3-19 14:30:45
2. You can send commands via the Serial Monitor:
- **Set time**: Enter `D2025-03-19T14:30:45` and press Enter.  
  The board responds with `Setting time...` and `Time set.`
- **Query time**: Enter `current time` and press Enter.  
  The board prints the current RTC time.
