# N20 Arduino Example 03: Ethernet TCP Server (W5500)

## Description
This example configures the N20 board with an onboard W5500 Ethernet module to act as a simple TCP server. The server listens on port **4196** and echoes back any data received from a connected client. This is useful for testing network connectivity and basic bidirectional communication.

## File Structure
- `src/N20_03_ethernet_tcp_server.ino` – Arduino source code.
- `precompiled/N20_03_ethernet_tcp_server.bin` – Precompiled firmware binary for ESP32‑S3.

## Using the Precompiled Binary
Flash `precompiled/N20_03_ethernet_tcp_server.bin` to address `0x0` using ESP Flash Download Tool or `esptool.py`.
...
The N20 board includes an onboard W5500 Ethernet module with the following connections:
...
Connect an Ethernet cable from the N20 board to your network router/switch.
...
3. From any device on the same network, connect to the N20 using a TCP client (e.g., `telnet 192.168.3.55 4196`).
