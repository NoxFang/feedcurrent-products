# DM16 Arduino Examples

This directory contains Arduino example sketches for the **KinCony DM16** multi‑function controller. The DM16 is based on the ESP32‑S3 and features 16‑channel DAC (via Modbus to ARM CPU), 16 digital inputs (PCF8575), 4 analog inputs (ADS1115), Ethernet (W5500), RS485, RTC (DS3231), SD card, OLED display, and Tuya WiFi module.

## Available Examples

| # | Example Folder | Description |
|---|----------------|-------------|
| 01 | `01_dac_16ch` | Demonstrates 16‑channel DAC control via Modbus, with button input handling (PCF8575) for adjusting outputs. |
| 02 | `02_read_digital_inputs` | Reads the 16 digital input ports (PCF8575 pins with custom mapping) and prints the binary state. |
...
The DM16 uses the following key pins:

| Function | GPIO / Address |
|----------|----------------|
| I2C SDA | GPIO8 |
| I2C SCL | GPIO18 |
| PCF8575 (inputs) | Address 0x22 (16 inputs, custom pin mapping) |
...
For complete pin definitions, refer to `../pin_definitions/DM16/DM16_pin_definition.md`.
