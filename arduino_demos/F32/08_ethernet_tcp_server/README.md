# F32 Arduino Example 08: Ethernet TCP Server (W5500)

## Description
This example configures the F32 board with an external W5500 Ethernet module to act as a simple TCP server. The server listens on port **4196** and echoes back any data received from a connected client. This is useful for testing network connectivity and basic bidirectional communication.

## File Structure
- `src/F32_08_ethernet_tcp_server.ino` – Arduino source code.
- `precompiled/F32_08_ethernet_tcp_server.bin` – Precompiled firmware binary for ESP32‑S3.

## Using the Precompiled Binary
Flash `precompiled/F32_08_ethernet_tcp_server.bin` to address `0x0` using ESP Flash Download Tool or `esptool.py`.

## Compiling from Source
- **Environment**: Arduino IDE with ESP32‑S3 board support.
- **Dependencies**: `Ethernet.h`, `SPI.h` (both included with the ESP32 Arduino core).
- **Steps**:
  1. Connect the W5500 module to the F32 board as per the pin table below.
  2. Open the `.ino` file, select `esp32-s3-devkitc-1`, and upload.

## Hardware Connections
Connect a W5500 Ethernet module to the F32 board using the following pins:
...
3. From any device on the same network, connect to the F32 using a TCP client (e.g., `telnet 192.168.3.55 4196`).
