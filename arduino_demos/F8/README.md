...
This directory contains Arduino example sketches for the **FeedCurrent F8** controller. The F8 is based on the ESP32‑S3 and features 8 relay outputs (PCF8575), 8 digital inputs (PCF8575), 4 analog inputs (ESP32 internal ADC), Ethernet (W5500), RS485, RTC (DS3231), SD card, OLED display, and Tuya WiFi module.

## Available Examples

| # | Example Folder | Description |
|---|----------------|-------------|
| 01 | `01_turn_on_off_relay` | Sequentially turns on/off the 8 relay outputs (PCF8575 pins 8‑15) with a 200 ms delay. |
| 02 | `02_read_digital_inputs` | Reads the 8 digital input ports (PCF8575 pins 0‑7) and prints the binary state every 500 ms. |
...
| 05 | `05_read_free_gpio` | Monitors 7 free GPIO pins (48,47,21,15,14,40,0) for state changes and prints the results. |
...
| 11 | `11_input_trigger_output` | Directly links the 8 digital inputs to the 8 relay outputs (active‑LOW logic). |

## Common Hardware Configuration

The F8 uses the following key pins:

| Function | GPIO / Address |
|----------|----------------|
| I2C SDA | GPIO8 |
| I2C SCL | GPIO18 |
| PCF8575 (inputs) | Address 0x24, pins 0‑7 |
| PCF8575 (outputs) | Address 0x24, pins 8‑15 |
...
For complete pin definitions, refer to `../pin_definitions/F8/F8_pin_definition.md`.
