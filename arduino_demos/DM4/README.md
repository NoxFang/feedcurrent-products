# DM4 Arduino Examples

This directory contains Arduino example sketches for the **FeedCurrent DM4** multi‑function controller. The DM4 is based on the ESP32‑S3 and features 4‑channel DAC (MCP4728), 4 digital inputs (PCF8574), 4 analog inputs (ADS1115), Ethernet (W5500), RS485, RTC (DS3231), SD card, OLED display, and Tuya WiFi module.

## Available Examples

| # | Example Folder | Description |
|---|----------------|-------------|
| 01 | `01_dac_mcp4728` | Demonstrates basic MCP4728 4‑channel DAC output. Sets each channel to a different value (full scale, half, quarter, zero). |
| 02 | `02_read_digital_inputs` | Reads the 4 digital input ports (PCF8574 pins P0–P3) and prints "KEYx PRESSED" when a key is pressed (active‑LOW). |
...
The DM4 uses the following key pins:
...
For complete pin definitions, refer to `../pin_definitions/DM4/DM4_pin_definition.md`.

## Usage

1. Open any example folder.
2. Copy the `.ino` file to your Arduino IDE or ESPHome project.
3. Install required libraries as needed (e.g., `PCF8575`, `Adafruit_MCP4728`, `DFRobot_ADS1115`, `RTC`, `U8g2`, etc.).
