# T32M Arduino Example 02: Read Digital Inputs

## Description
This example reads the status of all 32 digital inputs on the T32M board via two PCF8575 I/O expanders. The combined 32‑bit input state is printed as a binary string to the Serial Monitor every 500 ms.

- Inputs 1‑16: PCF8575 at I2C address `0x24`
- Inputs 17‑32: PCF8575 at I2C address `0x25`

## File Structure
- `src/T32M_02_read_digital_inputs.ino` – Arduino source code.
- `precompiled/T32M_02_read_digital_inputs.bin` – Precompiled firmware binary for ESP32‑S3.

## Using the Precompiled Binary
Flash `precompiled/T32M_02_read_digital_inputs.bin` to address `0x0` using ESP Flash Download Tool or `esptool.py`.

## Compiling from Source
- **Environment**: Arduino IDE with ESP32‑S3 board support.
- **Dependencies**: Install `PCF8575` library via Library Manager.
- **Steps**: Open the `.ino` file, select `esp32-s3-devkitc-1`, and upload.

## Hardware Connections
| Signal | GPIO |
|--------|------|
| SDA    | 48   |
| SCL    | 47   |

| PCF8575 | I2C Address | Input Channels |
|---------|-------------|----------------|
| First   | 0x24        | 1‑16           |
| Second  | 0x25        | 17‑32          |

## Expected Behavior
- On startup, the Serial Monitor (115200 baud) displays:  
  `FeedCurrent F32 32 channel input state 0:ON  1:OFF`
- Every 500 ms, a 32‑bit binary string is printed, e.g.:  
  `Input state: 00000000000000000000000000000000` (all inputs OFF)  
  `Input state: 10101010101010101010101010101010` (alternating pattern)
- A `0` indicates the input is **ON** (active‑LOW), a `1` indicates **OFF** (active‑HIGH). Refer to the pin definition for logic level details.

## Important Notes
- The example reads all 32 inputs and combines them into a single binary string.
- No debouncing is implemented; for noisy inputs, consider adding hardware filtering or software delay.

## Related Resources
- Pin definition: `../pin_definitions/T32M/T32M_pin_definition.md`