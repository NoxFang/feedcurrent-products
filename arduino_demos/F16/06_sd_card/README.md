# F16 Arduino Example 06: SD Card Operations

## Description
This example demonstrates basic SD card file operations using the F16 board's SPI interface. It automatically performs the following sequence:
...
## File Structure
- `src/F16_06_sd_card.ino` – Arduino source code.
- `precompiled/F16_06_sd_card.bin` – Precompiled firmware binary for ESP32‑S3.

## Using the Precompiled Binary
Flash `precompiled/F16_06_sd_card.bin` to address `0x0` using ESP Flash Download Tool or `esptool.py`.
...
## Hardware Setup
The F16 board includes a built‑in SD card slot. No external module is required.

| SD Card Slot Pin | ESP32‑S3 GPIO |
|------------------|---------------|
| CS               | 9             |
| MOSI             | 10            |
| MISO             | 12            |
...
Insert a micro SD card formatted as FAT32. The SD card slot is powered by the board's 3.3V supply.
...
## Important Notes
- **SPI Bus**: Uses HSPI (SPI2) with pins as listed.
