# B4 ESPHome Configuration

## Overview
This directory contains ESPHome YAML configurations for the **FeedCurrent B4** controller.  
The B4 is based on ESP32‑S3 and features:

- 4‑channel relay outputs (via PCF8574 @ 0x24, pins 4‑7)
...
| File | Description |
|------|-------------|
| `B4_esphome_with_tuya.yaml` | Includes Tuya WiFi module integration (cloud control via Tuya Smart app). |
| `B4_esphome_without_tuya.yaml` | Pure local ESPHome version (no Tuya). |

## Hardware Requirements
- FeedCurrent B4 controller (ESP32‑S3)
...
| PCF8574 | Address 0x24 | Pins 0‑3: digital inputs; pins 4‑7: relay outputs (active‑LOW) |
...
- **4‑channel relay control** via GPIO switch platform
...
4. **Flash** the configuration using ESPHome dashboard or CLI:
   ```bash
   esphome run B4_esphome_with_tuya.yaml
...
## Important Notes
- The PCF8574 chip controls both inputs and outputs; output pins are **active‑LOW** (relay ON when pin is LOW).
...
## Related Resources
- Pin definition: `../../pin_definitions/B4/B4_pin_definition.md`
- Arduino examples: `../../arduino_demos/B4/`
