# F8 (ESP32-S3) Pin Definitions

## Overview

The **F8** is an ESP32-S3 based smart controller with:
- **8 Channel Relay Output** (via PCF8575 I2C expander)
- **8 Channel Digital Input** (via same PCF8575)
- **4 Channel ADC** (2 voltage, 2 current, using ESP32 internal ADC)
- **Ethernet** (W5500 SPI)
- **RS485** (UART)
- **Tuya Module Interface** (UART)
- **1-Wire Bus** (4 channels with pull-up, 1 without)
...
|--------|-------------|-------------|
| Relay 1 | P8 | Relay output 1 |
| Relay 2 | P9 | Relay output 2 |
| Relay 3 | P10 | Relay output 3 |
| Relay 4 | P11 | Relay output 4 |
| Relay 5 | P12 | Relay output 5 |
| Relay 6 | P13 | Relay output 6 |
| Relay 7 | P14 | Relay output 7 |
| Relay 8 | P15 | Relay output 8 |
...
- **GPIO0 is not used** on the F8 hardware – avoid using it in your firmware
- ADC pins use ESP32‑S3 internal ADC; for precise measurements, calibration is recommended
- Relays are rated 10A (verify hardware version)
